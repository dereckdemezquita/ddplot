...
create_footer <- function (source_name, logo_image_path) {
  ...
  footer <- grid::grobTree(
    grid::linesGrob(
      x = grid::unit(c(0, 1), "npc"), y = grid::unit(1.1, "npc")
      ),
    grid::textGrob(
      source_name,
      x = 0.004,
...
#' Arrange alignment and save Dereck's ggplot chart
#'
#' Running this function will save your plot.
#' It will left align your title, subtitle and source, add the blocks at the bottom right and save it to your specified location.
...
#'  which needs to be a PNG file - defaults to a placeholder blocks image that sits within the data folder of your package
...
finalise_plot <- function(plot_name,
                          ...
                          width_pixels = 640,
                          height_pixels = 450,
                          logo_image_path = file.path(system.file("data", package = 'theme_dereck'), "placeholder.png")) {
